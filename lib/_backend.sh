...
backend_redis_create() {
  ...
  sudo su - root <<EOF
  usermod -aG docker deploybotusist
...
sudo su - deploybotusist << EOF
  cat <<[-]EOF > /home/deploybotusist/whaticket/backend/.env
NODE_ENV=
...
backend_start_pm2() {
  ...
  sudo su - deploybotusist <<EOF
  cd /home/deploybotusist/whaticket/backend
  pm2 start botusist/server.js --name whaticket-backend
EOF
